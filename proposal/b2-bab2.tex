...
Secara struktur, Transformer terdiri dari dua bagian utama, yaitu encoder dan decoder. Encoder bertugas mengolah teks input dan menghasilkan representasi yang sudah memahami konteks kalimat. Setiap lapisan encoder terdiri dari mekanisme multi-head self-attention yang diikuti oleh feed-forward neural network, serta dilengkapi dengan residual connection dan layer normalization agar proses pelatihan model tetap stabil. Decoder memiliki struktur yang mirip, namun digunakan untuk menghasilkan keluaran berdasarkan hasil dari encoder, terutama pada tugas seperti penerjemahan bahasa.

Keunggulan utama Transformer adalah kemampuannya melakukan pemrosesan data secara paralel, sehingga proses pelatihan menjadi lebih cepat dan efisien dibandingkan model sekuensial. Selain itu, Transformer tidak mengalami masalah vanishing gradient yang sering muncul pada RNN, serta mampu menangani teks dengan panjang yang bervariasi. Berbagai keunggulan tersebut menjadikan Transformer sebagai arsitektur dasar bagi banyak model bahasa modern dan sebagai standar utama dalam pengembangan sistem Natural Language Processing saat ini.
...
\begin{itemize}
    ...
    \item Menyediakan mekanisme visualisasi (word cloud) untuk setiap aspek yang membantu identifikasi pola sentimen
\end{itemize}
...
\begin{itemize}
    ...
    \item Variasi bahasa dan gaya penulisan yang lebih kaya karena berasal dari berbagai lokasi dan tipe usaha
\end{itemize}
...
\begin{itemize}
    \item Evaluasi model tidak hanya pada test set, tetapi juga pada online testing dengan data real dari minimal 10 usaha F\&B
